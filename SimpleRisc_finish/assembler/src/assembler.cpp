...
void Assembler::assemblerPass2(char* output) {
    ...
    for(i=0; i < mInstList.size(); i++) {
        ...
        //===================================================================================================
        else if( isSEVOperand(inst->opcode) ){
          //  SEV
          int SEVImm;
          bool imm = true;
          readSEVImm(inst->operands, SEVImm);
          inst->encoding = generateEncoding(inst->opcode, SEVImm, imm);
        }
        else if( isWFEOperand(inst->opcode) ){
          //WFE
          int WFEImm;
          bool imm = true;
          readWFEImm(inst->operands, WFEImm);
          inst->encoding = generateEncoding(inst->opcode, WFEImm, imm);
        ...
        //====================================================================================================
        else if( isProcidOperand(inst->opcode) ) {
            int destReg, src2;
            destReg = 0;
            bool imm = true;
            readProcidOperand(inst->operands, src2, imm);
...
//===================================================================================================
bool Assembler::isWFEOperand(Opcode opcode){
  if(opcode == WFE)
    return true;
  return false;
}
bool Assembler::isSEVOperand(Opcode opcode){
  if(opcode == SEV)
    return true;
  return false;
}
bool Assembler::isProcidOperand(Opcode opcode){
  return opcode == Procid ? true : false;
}
//===================================================================================================
...
//===================================================================================================
void Assembler::readSEVImm(std::string &input, int &MultiImm){
  removeInitialWhiteSpace(input);
  //size_t pos = input.find_first_of(" ");
  if(input == ""){
    std::cerr << "Didn't Find the immediate for  WFE or SEV"<< std::endl;
    exit(-1);
  }
  std::string multiNum = input;
  lowerCase(multiNum);
  MultiImm = binaryToDec(multiNum);
}
void Assembler::readWFEImm(std::string &input, int &MultiImm){
  removeInitialWhiteSpace(input);
  //size_t pos = input.find_first_of(" ");
  if(input == ""){
    std::cerr << "Didn't Find the immediate for  WFE or SEV"<< std::endl;
...
//=======================================================================================================
void Assembler::readProcidOperand(std::string &input, int &regIndex, bool &imm){
  removeInitialWhiteSpace(input);
  if(input == ""){
    std::cerr << "Didn't find the source or immediate for Procid" << std::endl;
    exit(-1);
  }
  if(input[0] == 'r'){
    imm = false;
    if(input[2] >= '0' && input[2] <= '5'){
        regIndex = input[2] - '0' + 10 * (input[1] - '0');
    } else {
        //single digit operand
        regIndex = input[1] - '0';
    }
...
uint32 Assembler::generateEncoding(Opcode opcode, int Immval, bool imm){
  return ((opcode<<27) | (Immval & 0x7FFFFFF) | (1<<26));
}
...
unsigned int Assembler::binaryToDec(std::string number){
    int result = 0, pow = 1;

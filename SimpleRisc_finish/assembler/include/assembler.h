...
class Assembler {
        ...
        private:
            ...
            void readSEVImm(std::string &input, int &MultiImm);
            void readWFEImm(std::string &input, int &MultiImm);
            void readMemoryOffset(std::string &input, int &offset);
            void readRegisterOperand(std::string &input, int &regIndex, bool &imm);
            void readProcidOperand(std::string &input, int&regIndex, bool &imm);
            ...
            uint32 generateEncoding(Opcode opcode, int Immval, bool imm);
            ...
            bool isSEVOperand(Opcode opcode);
            bool isWFEOperand(Opcode opcode);
            bool isProcidOperand(Opcode opcode);
            int readNumber(std::string &);
            unsigned int binaryToDec(std::string number);

...
Core::Core(): if_of(pipeline), of_ex(pipeline), ex_ma(pipeline), ma_rw(pipeline), PC(false){
...
void Core::setInitCore(){
	initCore = true;
}

bool Core::getInitCore(){
	return initCore;
}

// void Core::setOutFileAndMem( Memory &Mem){
// 	MEM = Mem;
// 	//output_file = outFile;
// }
...
void Core::run_simplesim(const unsigned int pVar){
	printVar = pVar;
